...
observation_space = (lander_velocity_x, lander_velocity_y)
...
def create_environment():
    ...
    Q = np.zeros((len(lander_velocity_x)+1, len(lander_velocity_y)+1,  env.action_space.n))
    hit_table = np.zeros((len(lander_velocity_x)+1, len(lander_velocity_y)+1))
...
def choose_actions(state, env, Q, digitized_state):
    ...
    # if np.random.random() < epsilon:if -0.05 <= angle <= 0.05:
...
def train(total_episodes, max_steps, env, Q, epsilon, hit_table):
    ...
    for episode in range(total_episodes):
        ...
        digitized_state_1_a = digitize_states(math.tanh(state1[2]), lander_velocity_x)  # binning state value
        digitized_state_1_b = digitize_states(math.tanh(state1[3]), lander_velocity_y)  # binning state value
        ...
        action1 = choose_actions(state1, env, Q_old, digitized_state_1)  # choosing an action based on the state
        ...
        while not episode_finished:
            ...
            digitized_state_2_a = digitize_states(math.tanh(state2[2]), lander_velocity_x)  # binning state value
            digitized_state_2_b = digitize_states(math.tanh(state2[3]), lander_velocity_y)  # binning state value
            ...
            action2 = choose_actions(state2, env, Q_old, digitized_state_2)  # choosing an action based on the state
    ...
    f = open("lunar_lander_pretrained.pkl", "wb")
    pickle.dump(Q, f)
    f.close()
...
def play(Q):
    ...
    # Read the Q table from the File
    f = open("lunar_lander_pretrained.pkl", "rb")
    Q = pickle.load(f)
    f.close()
