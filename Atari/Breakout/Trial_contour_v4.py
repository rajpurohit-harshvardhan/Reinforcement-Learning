...
def create_environment(render):
    ...
    Q = np.zeros((len(paddle_position)+1, len(ball_position_y)+1, len(ball_predicted_x)+1, len(ball_velocity_y)+1, env.action_space.n))
    return env, Q
...
def fetch_ball_and_paddle_info(state, frame_gap=1):
    ...
    velocities, predicted_ball_pos_x = calculate_predicted_values(ball_pos, reference_ball_pos, 4)
...
def train(total_episodes, max_steps, env, Q, epsilon):
    ...
    for episode in range(total_episodes):
        ...
        ball_pos, paddle_pos, ball_velocity, x_predicted = fetch_ball_and_paddle_info(state, frame_gap=1)
        ...
        state1 = (state_paddle, state_ball_y, state_predicted_x, state_ball_velocity_y)
        ...
        while not terminated:
            ...
            _, _, _, score = fetch_information_from_ram(env)
            ball_pos, paddle_pos, ball_velocity, x_predicted = fetch_ball_and_paddle_info(state2, frame_gap=1)
            ...
            state2 = (newstate_paddle, newstate_ball_y, newstate_predicted_x, newstate_ball_velocity_y)
...
def play():
    ...
    f = open("breakout_v4.pkl", "rb")  # Read the Q table from the File
    Q = pickle.load(f)
    f.close()
    ...
    for i in range(loop_for):
        ...
        ball_pos, paddle_pos, ball_velocity, x_predicted = fetch_ball_and_paddle_info(reset_state, frame_gap=1)
        state_paddle = np.digitize(paddle_pos[0], paddle_position, True)
        state_ball_x = np.digitize(ball_pos[0], ball_position_x, True)
        state_ball_y = np.digitize(ball_pos[1], ball_position_y, True)
        state_predicted_x = np.digitize(x_predicted, ball_predicted_x, True)
        state_ball_velocity_y = np.digitize(ball_velocity[1], ball_velocity_y, True)
        state = (state_paddle, state_ball_y, state_predicted_x, state_ball_velocity_y)
        ...
        while not episode_terminated:
            ...
            newstate_predicted_x = np.digitize(x_predicted, ball_predicted_x, True)
            newstate_ball_velocity_y = np.digitize(ball_velocity[1], ball_velocity_y, True)
            state = (newstate_paddle, newstate_ball_y, newstate_predicted_x, newstate_ball_velocity_y)
