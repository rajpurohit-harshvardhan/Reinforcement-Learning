...
def calculate_velocities_and_predicted_position(ball, frame_gap=4):
    ...
    velocities, predicted_ball_pos_x = calculate_predicted_position(ball, reference_ball_pos, 4)
...
def train(total_episodes, max_steps, env, Q, epsilon):
    ...
    for episode in range(total_episodes):
        ...
        ball_velocity, x_predicted = calculate_velocities_and_predicted_position(ball, frame_gap=4)
    ...
    f = open("breakout_v5_with_x_position.pkl", "wb")
    pickle.dump(Q, f)
    f.close()
...
def play(render):
    ...
    f = open("breakout_v5_with_x_position_5k.pkl", "rb")  # Read the Q table from the File
    Q = pickle.load(f)
    f.close()
